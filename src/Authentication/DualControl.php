...
final class DualControl
{
    ...
    public function __construct(string $model, string $action)
    {
        ...
        $this->makerUserId = (string)(Auth::id() ?? '');

        $this->tokenGuid = $this->newGuid();
    }

    /**
     * Blocks execution and creates approval requests even if no DB rule exists.
     */
    public function forceCreateApprovalRequest(): void
    {
        $this->dualRequired = true;
        
        // Attempt to find a default group (e.g. System Admin Group 1)
        if ($this->groupId === null) {
            $this->groupId = 1; 
        }

        $this->createApprovalRequests(0, "Code-Defined Rule");
    }
        ...
        foreach ($checkers as $c) {
            ...
            $sql = "INSERT INTO mx_dual_activity_log
                    (opt_mx_dual_activity_id, opt_mx_login_credential_id, txt_token, txt_column_value,
                     ...
                     txt_payload, txt_module_name, txt_method_name)
                    VALUES
                    (:aid, :cid, :token, :ref, GETDATE(), :by, 0, :row, :payload, :module, :method)";

            $stmt = $this->db->prepare($sql);
            $stmt->execute([
                ':aid'     => ($dualActivityId > 0) ? $dualActivityId : null,
                ':cid'     => $checkerId,
                ':token'   => $token,
                ':ref'     => $referenceValue,
                ':by'      => $this->makerUserId,
                ':row'     => $this->newGuid(),
                ':payload' => json_encode($this->readRequestPayload()),
                ':module'  => $this->model,
                ':method'  => $this->action,
            ]);
