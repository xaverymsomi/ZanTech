...
class DataView
{
    ...
    public function displayTBody(array $records, string $class, string $table, array $hidden = [], array $actions = [], $labelSize = 'big'): string
    {
        ...
        foreach ($records as $index => $row) {
            ...
            foreach ($row as $key => $value) {
                ...
                $label = htmlspecialchars($this->columnLabels[$key] ?? $key, ENT_QUOTES, 'UTF-8');
    ...
    private function renderActionButton(array $action, string $rowValue): string
    {
        ...
        $btnIcon = htmlspecialchars($action['icon'] ?? 'fa-edit', ENT_QUOTES, 'UTF-8');
        $btnName = htmlspecialchars($action['name'] ?? 'Action', ENT_QUOTES, 'UTF-8');
        $type = htmlspecialchars($action['type'] ?? 'edit', ENT_QUOTES, 'UTF-8');
        
        $escapedValue = htmlspecialchars($rowValue, ENT_QUOTES, 'UTF-8');
        $escapedTitle = $btnName;
    ...
    {
        $icon = htmlspecialchars($action['icon'] ?? 'fa-circle', ENT_QUOTES, 'UTF-8');
        $name = htmlspecialchars($action['name'] ?? 'Action', ENT_QUOTES, 'UTF-8');
        $type = htmlspecialchars($action['type'] ?? 'edit', ENT_QUOTES, 'UTF-8');
        $color = strtolower((string)($action['color'] ?? ''));
