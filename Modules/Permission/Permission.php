...
class Permission extends Controller
{
    ...
    public function getUserGroups(): void
    {
        try {
            ...
            if ($errors) {
                $this->jsonError('Validation error', 422, ['errors' => $errors]);
            }

            $result = $this->service->getUserGroups($payload['domain'], $payload['id']);
            $this->jsonSuccess(200, 'User groups loaded', ['data' => $result]);

        } catch (Exception $e) {
            $this->service->logError('getUserGroups', $e);
            $this->jsonError('Failed to load user groups', 500);
        }
    }

    /** POST assign user groups */
    public function postUserGroup(): void
    {
        try {
            $payload = PermissionValidator::readJsonBody();
            ...
            if (($result['status'] ?? 200) >= 400) {
                $this->jsonError($result['title'] ?? 'Error', $result['status'] ?? 500);
            }

            $this->jsonSuccess(200, $result['title'] ?? 'User groups saved');

        } catch (Exception $e) {
            $this->service->logError('postUserGroup', $e);
            $this->jsonError('An error occurred while saving groups', 500);
        }
    }
    ...
    public function getGroupPermissions(): void
    {
        try {
            ...
            $this->jsonSuccess(200, 'Group permissions loaded', ['data' => $result]);

        } catch (Exception $e) {
            $this->service->logError('getGroupPermissions', $e);
            $this->jsonError('Failed to load group permissions', 500);
        }
    }

    /** POST assign group permissions */
    public function postGroupPermission(): void
    {
        try {
            $payload = PermissionValidator::readJsonBody();
            ...
            $this->jsonSuccess(200, $result['title'] ?? 'Group permissions updated');

        } catch (Exception $e) {
            $this->service->logError('postGroupPermission', $e);
            $this->jsonError('Failed to update group permissions', 500);
        }
    }
    ...
    public function getUserPermissions(): void
    {
        try {
            ...
            $this->jsonSuccess(200, 'User permissions loaded', ['data' => $result]);

        } catch (Exception $e) {
            $this->service->logError('getUserPermissions', $e);
            $this->jsonError('Failed to load user permissions', 500);
        }
    }

    /** POST assign user permissions */
    public function postUserPermission(): void
    {
        try {
            $payload = PermissionValidator::readJsonBody();
            ...
            $this->jsonSuccess(200, $result['title'] ?? 'User permissions updated');

        } catch (Exception $e) {
            $this->service->logError('postUserPermission', $e);
            $this->jsonError('Failed to update user permissions', 500);
        }
    }
    ...
    public function saveGroup(): void
    {
        try {
            ...
            $this->jsonSuccess(200, $result['title'] ?? 'Group saved');

        } catch (Exception $e) {
            $this->service->logError('saveGroup', $e);
            $this->jsonError('Failed to save group', 500);
        }
    }
    ...
    public function savePermission(): void
    {
        try {
            ...
            $this->jsonSuccess(200, $result['title'] ?? 'Permission saved');

        } catch (Exception $e) {
            $this->service->logError('savePermission', $e);
            $this->jsonError('Failed to save permission', 500);
        }
    }
    ...
    public function saveSection(): void
    {
        try {
            ...
            $this->jsonSuccess(200, $result['title'] ?? 'Section saved');

        } catch (Exception $e) {
            $this->service->logError('saveSection', $e);
            $this->jsonError('Failed to save section', 500);
        }
    }
    ...
    public function loadData(): void
    {
        try {
            ...
            $this->jsonSuccess(200, 'Permission data loaded', ['data' => $data]);

        } catch (Exception $e) {
            $this->service->logError('loadData', $e);
            $this->jsonError('Failed to load permission data', 500);
        }
    }
}
