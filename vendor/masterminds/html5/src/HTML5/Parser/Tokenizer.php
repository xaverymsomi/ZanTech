 ...
 * semantic units. From the tokenizer, data is emitted to an event helpers,
...
class Tokenizer
{
     ...
     * it a scanner (input) and an event helpers (output), and then calling
     * the Tokenizer::parse() method.`
     *
     * @param Scanner      $scanner      A scanner initialized with an input stream.
     * @param EventHandler $eventHandler An event helpers, initialized and ready to receive events.
     * @param string       $mode
     */
     ...
     * Tokens are emitted into the event helpers.
     *
     * Tokenizing will continue until the document is completely
     * read. Errors are emitted into the event helpers, but
     ...
     *
     * Normally, setting is done by the event helpers via a special return code on
     ...
     * not Quirksmode is enabled on the event helpers.
     *
