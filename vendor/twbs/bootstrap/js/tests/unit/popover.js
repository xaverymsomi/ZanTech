$(function () {
  ...
  QUnit.test('should detach popover content rather than removing it so that event handlers are left intact', function (assert) {
    assert.expect(1)
    var $content = $('<div class="content-with-helpers"><a class="btn btn-warning">Button with event helpers</a></div>').appendTo('#qunit-fixture')

    var handlerCalled = false
    $('.content-with-helpers .btn').on('click', function () {
      handlerCalled = true
    })
      ...
      .one('shown.bs.popover', function () {
        $div
          .one('hidden.bs.popover', function () {
            $div
              .one('shown.bs.popover', function () {
                $('.content-with-helpers .btn').trigger('click')
                $div.bootstrapPopover('destroy')
                assert.ok(handlerCalled, 'content\'s event helpers still present')
                done()
              })
              .bootstrapPopover('show')
          })
